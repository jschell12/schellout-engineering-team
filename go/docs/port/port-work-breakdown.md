...
- **T5.6 — approval gate + resume** (`orch/approval_gate.go`, `orch/resume.go`): port `app.py:355-471,759-838` (hax plan-approval, engages only when HAX_API_KEY set) using `hitl` poll primitive; and `app.py:2081-2134` resume_build (load `<artifacts>/execution/checkpoint.json`, reconstruct minimal plan_result, call `execute(resume=true)`). (~500)
